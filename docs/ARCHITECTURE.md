...
- **Scaling back down**: the burst tier consolidates `WhenEmptyOrUnderutilized`, so once
  traffic recedes it actively bin-packs and drains Spot nodes, rather than waiting for
  them to empty on their own. The baseline tier's higher NodePool weight means pods
  evicted off Spot reschedule onto on-demand as soon as the (now-shrunk) ceiling has
  room, so capacity naturally drains back to the cheaper, steadier tier post-spike.
...
- **Considered, not used**:
  ...
  - **Amazon Managed Prometheus + Amazon Managed Grafana**: the more direct answer to
    ...
    dashboards.
  - **Reserved Instances / Savings Plans for the on-demand-baseline tier**: its capacity
    is steady-state and predictably sized, exactly the profile RIs/Savings Plans
    discount. Not purchased here; a real Day-2 cost lever left for later.
...
- **Versioning: one minor version behind latest by default.** Applies to the EKS
  ...
  **Kubernetes support: STANDARD, not EXTENDED** — staying near-latest already keeps
  the cluster inside standard support's window.
...
- **Edge protection**: an IP-allowlist security group on the ALB, not AWS WAFv2.
  Enterprise: WAFv2 managed rule groups plus a rate-based rule, in front of (or instead
  of) the allowlist, since WAF can't be the only edge control once real users aren't
  coming from a handful of known IPs.
...
- **Capacity buffer**: `capacity-buffer` pre-warms a single node's worth of
  headroom. Enterprise: size the buffer to cover several nodes, with
  upscale/downscale timing for scaling object tuned to real traffic patterns
